...
// MaxExptime is the maximum time to send from a client before
// the timestamp is considered an absolute unix
// timestamp.
const MaxExptime = 60 * 60 * 24 * 30 // 30 days
...
// IsExpired checks if an Item is expired based on it's Ttl.
...
// SetExpires sets the Ttl and Expires based on the exptime send from
